...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const tBenefit = await getTranslations({ locale, namespace: "ownerBenefits" });
  return {
    title: `${tBenefit(`${slug}.titre`)} | ZenVilla`,
    description: tBenefit(`${slug}.description`),
  };
}
...
export default async function ProprietaireAvantagePage({ params }: Props) {
  ...
  const tBenefit = await getTranslations({ locale, namespace: "ownerBenefits" });
  const tCommon = await getTranslations({ locale, namespace: "Common" });
  const titre = tBenefit(`${slug}.titre`);
  const description = tBenefit(`${slug}.description`);
  const details = tBenefit.raw(`${slug}.details`) as readonly string[];
      ...
      <section className="relative hero-bandeau flex flex-col justify-center overflow-hidden">
        ...
        <div className="relative z-10 mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-12">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl sm:text-4xl font-serif font-semibold text-white">{titre}</h1>
