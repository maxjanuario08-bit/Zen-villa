...
export default async function Footer() {
  ...
  return (
    <footer className="bg-lagoon-dark text-white">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12 lg:py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12">
          ...
          {/* Services */}
          <div>
            <h3 className="text-sm font-semibold uppercase tracking-wider text-sand-light/90 mb-4">
              {tFoot("servicesTitle")}
            </h3>
            <ul className="space-y-2">
              {serviceLinks.map((entry) => {
                const label = "navKey" in entry ? tNav(entry.navKey) : tFoot(entry.footKey);
                return (
                  <li key={entry.href}>
                    <Link
                      href={entry.href}
          ...
          <div>
            ...
            <ul className="space-y-3">
              ...
              <li>
                <Link href="/contact#faq" className="text-sm text-white/80 hover:text-white transition-colors">
                  {tFoot("faq")}
                </Link>
              </li>
            </ul>
          ...
          <div className="space-y-8">
            ...
            {/* Langues (icônes drapeaux + phrase + sélecteur) */}
            <div className="space-y-3">
              <p className="text-xs uppercase tracking-wider text-sand-light/80">{tLang("spoken")}</p>
              <p className="text-sm text-white/85 flex flex-wrap items-center gap-2 leading-relaxed">
                <span aria-hidden className="text-lg">
                  🇫🇷 🇬🇧 🇪🇸
                </span>
                <span>{tLang("spokenSentence")}</span>
              </p>
