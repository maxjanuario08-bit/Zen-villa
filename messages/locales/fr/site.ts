const site = {
  Titles: {
    contactPage: "Contact – Devis et informations",
    contactDesc:
      "Contactez Zenvilla pour un devis personnalisé. Réponse rapide 7j/7. Propriétaires de villas et voyageurs en Corse Sud.",
    contactOg: "Demandez un devis ou des informations. Notre équipe vous répond rapidement.",
    demanderPage: "Demander une prestation – Transport, petit-déj, activités",
    demanderDesc:
      "Demandez une prestation pour votre séjour en villa : transport, petit-déjeuner, activités nautiques, balades équestres, location équipement. ZenVilla Corse Sud.",
    demanderOg:
      "Transport, petit-déjeuner, activités nautiques, balades à cheval… Demandez votre prestation en Corse Sud.",
    packsOg: "Formules conciergerie pour propriétaires : essentiel, gestion complète ou à la carte, annonces et réservations.",
    ownersOg:
      "Conciergerie pour villas d'exception : accueil, ménage, annonces et assistance 7j/7.",
    voyageursOg:
      "Services sur mesure pour voyageurs : transport, petit-déjeuner, activités, location de matériel nautique.",
    pvMetaTitle: "Conciergerie Porto-Vecchio – Gestion villa & Airbnb",
    pvMetaDesc:
      "Conciergerie pour villas à Porto-Vecchio. Gestion Airbnb, Santa Giulia, Palombaggia. ZenVilla vous accompagne en Corse Sud.",
    pvOgTitle: "Conciergerie Porto-Vecchio | ZenVilla – Gestion villas Corse Sud",
    pvOgDesc: "Confiez la gestion de votre villa à Porto-Vecchio. Expertise locale, gestion complète.",
    sgMetaTitle: "Conciergerie Santa Giulia – Gestion villa & Airbnb Porto-Vecchio",
    sgMetaDesc:
      "Conciergerie pour villas à Santa Giulia. Gestion Airbnb, plage Santa Giulia. ZenVilla, expertise locale Corse Sud.",
    sgOgTitle: "Conciergerie Santa Giulia | ZenVilla – Gestion villas",
    sgOgDesc: "Conciergerie Santa Giulia et Porto-Vecchio. Gestion complète de votre villa.",
    mentionsMetaTitle: "Mentions légales",
    mentionsMetaDesc:
      "Mentions légales et informations juridiques du site Zenvilla – Conciergerie Corse Sud.",
    politiqueMetaTitle: "Politique de confidentialité",
    politiqueMetaDesc:
      "Politique de confidentialité, protection des données (RGPD) et gestion des cookies du site Zenvilla – Conciergerie Corse Sud.",
  },
  Home: {
    heroTitle: "Confiez-nous vos clés, nous gérons le reste.",
    heroSubtitle:
      "Notre conciergerie digitale et personnalisée gère tout en totale transparence pour vous libérer l'esprit.",
    heroPillars: "RÉACTIVITÉ • FLEXIBILITÉ • PROXIMITÉ",
    heroAlt: "Baie de Corse Sud – vue mer turquoise",
    whyTitle: "Pourquoi faire confiance à Zenvilla ?",
    whyLead:
      "Parce que gérer une propriété à distance devrait être facile. Notre conciergerie réinvente la gestion de vos villas à Santa Giulia et Porto-Vecchio. En combinant une expertise locale unique et une transparence totale. Vous n'avez plus qu'à vous libérer l'esprit : on s'occupe de tout.",
    whyPillars: [
      {
        title: "📍 Proximité & Expertise",
        body: "Une équipe présente au quotidien sur le secteur de Santa Giulia et Porto-Vecchio pour une réactivité et une maintenance 7j/7.",
      },
      {
        title: "🔑 Clés en main (A à Z)",
        body: "De la création et l'optimisation de l'annonce à l'accueil des voyageurs en passant par un ménage professionnel.",
      },
      {
        title: "🌐 Transparence",
        body: "Un suivi rigoureux de vos plannings, de vos réservations et de l'état de votre villa en temps réel, où que vous soyez.",
      },
    ],
    servicesTitle: "Nos services",
    servicesHint: "En savoir plus →",
    ctaTrustVilla: "Confier ma villa",
    ownersCtaTitle: "Prêt à confier votre villa ?",
    ownersCtaBody:
      "Demandez un devis : Zen Tranquillité, Zen Intégral ou formule à la carte.",
    reviewsTitle: "Avis voyageurs",
    reviewsLead: "Quelques retours sur Mini Villa Pinson et l’accueil Zenvilla à Santa Giulia.",
    reviewsSource: "Avis type Google · extraits fictifs pour la mise en page",
    reviews: [
      {
        name: "Claire M.",
        place: "Lyon",
        text: "Villa impeccable, 5 minutes de la plage. L’accueil a tout expliqué sans stress.",
      },
      {
        name: "Marco R.",
        place: "Milan",
        text: "Calme, propre, super terrasse. Conciergerie réactive pour le kayak.",
      },
      {
        name: "Sophie & Julien",
        place: "Paris",
        text: "Mini Villa Pinson est parfaite en couple. Check-in fluide, livret très utile.",
      },
      {
        name: "Elena G.",
        place: "Barcelone",
        text: "Santa Giulia comme sur les photos. Équipe locale, on se sent accompagnés.",
      },
      {
        name: "Thomas B.",
        place: "Bruxelles",
        text: "Propre à l’arrivée et au départ. WhatsApp suivi du début à la fin.",
      },
    ],
  },
  Packs: {
    metaTitlePacks: "Formules – Zen Tranquillité, Zen Intégral & à la carte",
    metaDescPacks:
      "Découvrez nos formules conciergerie : Zen Tranquillité, Zen Intégral et à la carte pour propriétaires en Corse Sud.",
    ogTitle: "Nos formules | ZenVilla – Conciergerie Corse Sud",
    heroTitle: "Nos formules",
    heroSubtitle:
      "Choisissez la formule adaptée à vos besoins : Zen Tranquillité, Zen Intégral ou à la carte.",
    heroAlt: "Côte corse – plages et eaux turquoise",
    badgePopular: "Populaire",
    zenTTitle: "Zen Tranquillité",
    zenTSubtitle: "Formule essentielle",
    zenTEssential: [
      "Check-in / Check-out des voyageurs",
      "Ménage complet et préparation de la villa",
      "Gestion du linge",
      "Tout suivi via notre application digitale",
      "Assistance locale en cas de besoin",
      "Cadeau de bienvenue pour vos voyageurs",
    ],
    zenPTitle: "Zen Intégral",
    zenPSubtitle: "Formule tout-en-un",
    zenPEssential: [
      "Toute la formule essentielle +",
      "Création et diffusion des annonces Airbnb, Booking, Abritel…",
      "Shooting photos pro & mise en valeur de la villa",
      "Gestion du calendrier et des réservations",
      "Réponses aux voyageurs 7j/7",
    ],
    carteTitle: "À la carte",
    carteSubtitle: "Au choix",
    carteEssential: [
      "Prestations modulaires : ménage, linge, check-in / check-out, dépannage, maintenance…",
    ],
  },
  Owners: {
    metaTitleOwners: "Propriétaires – Conciergerie villa Santa Giulia & Porto-Vecchio",
    metaDescOwners:
      "Confiez votre villa à ZenVilla. Gestion complète pour une location sereine, expertise locale Santa Giulia et Porto-Vecchio.",
    flyerKickerTop: "Conciergerie",
    flyerKickerBot: "Corse Sud",
    heroTitle: "Confiez-nous vos clés,\nnous gérons le reste.",
    heroSubtitle:
      "Notre conciergerie digitale et personnalisée gère tout en totale transparence pour vous libérer l'esprit.",
    flyerPillars: "Réactivité • Flexibilité • Proximité",
    heroAlt: "Villa Corse Sud – Santa Giulia Porto-Vecchio",
    whySectionTitle: "Pourquoi ZenVilla ?",
    whyBullets: [
      "📍 Expertise locale unique • Santa Giulia & Porto-Vecchio",
      "🔑 Gestion complète de A à Z • Confiez-nous vos clés, nous gérons le reste",
      "🌐 Création & Optimisation • Valorisation de votre annonce sur les plateformes",
      "🗓️ Gestion des réservations • Suivi rigoureux des plannings et des voyageurs",
      "🛎️ Accueil & États des lieux • Check-in/out méticuleux pour chaque séjour",
      "✨ Ménage & Blanchisserie • Entretien professionnel haut de gamme",
      "🛠️ Maintenance & Dépannage • Assistance d'urgence réactive 7j/7",
      "👑 Services voyageurs sur-mesure • Prestations et activités personnalisées",
      "🛡️ Conciergerie digitale • Totale transparence pour vous libérer l'esprit",
    ],
    processTitle: "Notre périmètre pour votre villa",
    flyerPhases: [
      {
        line1: "Création & optimisation",
        line2: "de votre annonce",
      },
      {
        line1: "Gestion des réservations",
        line2: "& planification",
      },
      {
        line1: "Accueil, check-in / out",
        line2: "& états des lieux",
      },
      {
        line1: "Ménage professionnel",
        line2: "& blanchisserie",
      },
      {
        line1: "Maintenance & dépannage",
        line2: "d'urgence (7j/7)",
      },
      {
        line1: "Services & activités",
        line2: "voyageurs personnalisés",
      },
    ],
    advantagesSectionTitle:
      "Pourquoi confier votre villa à ZenVilla ?",
    ctaTitle: "Prêt à confier votre villa ?",
    ctaSubtitle:
      "Demandez un devis : Zen Tranquillité, Zen Intégral ou formule à la carte.",
  },
  GuestsIndex: {
    metaTitleGuests: "Voyageurs – Prestations & services villa Corse",
    metaDescGuests:
      "Profitez de prestations haut de gamme pour votre séjour en villa en Corse Sud : transport, petit-déjeuner, activités nautiques, location équipement et véhicules.",
    heroTitle: "Prestations aux voyageurs",
    heroSubtitle:
      "Pour un séjour en tout confort en villa en Corse Sud.",
    heroAlt: "Côte méditerranéenne – mer turquoise et rochers",
    servicesHeading: "Nos services pour votre séjour",
    hint: "En savoir plus →",
    ctaTitle: "Un séjour sur mesure ?",
    ctaBody:
      "Que ce soit pour organiser une sortie en mer, une livraison de courses ou une balade à cheval, notre équipe est à votre écoute.",
  },
  DemanderPrestation: {
    heroTitle: "Demander une prestation",
    heroSubtitle:
      "Transport, petit-déjeuner, activités nautiques… Dites-nous ce dont vous avez besoin pour votre séjour.",
    formTitle: "Formulaire de demande",
    fieldService: "Prestation souhaitée *",
    optionPlaceholder: "Sélectionnez une prestation",
    fieldName: "Nom *",
    fieldEmail: "Email *",
    fieldPhone: "Téléphone *",
    nomPlaceholder: "Votre nom",
    emailPlaceholder: "vous@email.fr",
    phonePlaceholder: "6 12 34 56 78",
    fieldPlace: "Villa ou lieu du séjour",
    fieldDates: "Dates souhaitées",
    fieldMsg: "Message / précisions",
    submit: "Envoyer ma demande",
    sending: "Envoi…",
    successFormspree:
      "Votre message a bien été envoyé. Nous vous recontactons vite.",
    successMailto: "Ouverture de votre messagerie… Envoyez le mail pour valider.",
    error: "Erreur lors de l'envoi. Merci de réessayer ou de nous joindre par téléphone.",
    sideTitle: "Un besoin précis ?",
    sideParagraph:
      "Écrivez-nous ou appelez-nous : nous préparons un devis sous 24 à 48h.",
    guestLabel: "Voyageur — prestation",
    bannerAlt: "Prestations voyageurs – Corse Sud",
    asideTitleDirect: "Ou contactez-nous directement",
    asideLead:
      "Préférez un appel ou un email ? Nous sommes disponibles pour répondre à vos questions.",
    callCaption: "Appelez-nous",
    emailCaption: "Envoyez un email",
    whatsappCaption: "WhatsApp",
    whatsAction: "Nous contacter",
    placePlaceholder: "Santa Giulia, Porto-Vecchio…",
    datesPlaceholder: "Ex: 15-22 juillet",
    msgPlaceholder: "Décrivez votre besoin…",
    errSend:
      "Une erreur s'est produite. Utilisez l'email ou le téléphone ci-contre.",
  },
  Legal: {
    mentionsLead:
      "Conformément aux dispositions des articles 6-III et 19 de la Loi n° 2004-575 du 21 juin 2004 pour la Confiance dans l'économie numérique.",
    lastUpdatedPrefix: "Dernière mise à jour :",
  },
  ConciergePV: {
    heroTitle: "Conciergerie villa Porto-Vecchio",
    heroSubtitle:
      "Gestion complète de votre villa à Santa Giulia, Palombaggia et Porto-Vecchio — en toute sérénité.",
    heroImgAlt:
      "Conciergerie villa Porto-Vecchio – plages Santa Giulia",
    bodyTitle:
      "Conciergerie villa Porto-Vecchio – ZenVilla",
    bodyLead:
      "ZenVilla est votre conciergerie de référence à Porto-Vecchio et en Corse Sud. Nous gérons votre villa à Santa Giulia, Palombaggia ou ailleurs dans la région : annonces Airbnb et Booking, check-in/check-out, ménage, maintenance et assistance voyageurs 7j/7.",
    bodyNext:
      "Confiez-nous la gestion de votre bien pour une location sereine tout au long de l'année.",
    ctaTitle:
      "Prêt à confier votre villa à Porto-Vecchio ?",
  },
  ConciergeSG: {
    heroTitle: "Conciergerie villa Santa Giulia",
    heroSubtitle:
      "Expertise locale pour votre villa à Santa Giulia — gestion des locations et plages d'exception.",
    heroImgAlt:
      "Conciergerie villa Santa Giulia – plage Porto-Vecchio",
    bodyTitle:
      "Conciergerie Santa Giulia – Gestion de villas en Corse",
    bodyLead:
      "Santa Giulia est l'une des plus belles plages de Corse. Si vous avez une villa dans ce secteur, ZenVilla vous propose une conciergerie complète : gestion Airbnb et Booking, accueil des voyageurs, ménage et disponibilité 7j/7.",
    bodyNext:
      "Notre équipe locale connaît Santa Giulia et Porto-Vecchio — confiez-nous votre villa en toute confiance.",
    ctaTitle: "Villa à Santa Giulia ? Confiez-la à ZenVilla",
  },
  ContactPage: {
    heroTitle: "Contact",
    heroSubtitle:
      "Une question, un projet ? Notre équipe vous répond rapidement.",
    heroAlt:
      "Contacter ZenVilla – eaux turquoise en Corse du Sud et conciergerie villas",
    fastReply: "Réponse rapide – 7j/7",
    formTitle: "Demander un devis",
    lblName: "Nom *",
    phName: "Votre nom",
    lblEmail: "Email *",
    phEmail: "votre@email.fr",
    lblPhone: "Téléphone *",
    phPhone: "6 12 34 56 78",
    lblCity: "Ville / zone en Corse",
    phCity: "Bonifacio, Porto-Vecchio…",
    lblType: "Type de bien",
    typeOpt: "Sélectionnez",
    typeVilla: "Villa",
    typeApt: "Appartement",
    typeMaison: "Maison",
    typeAutre: "Autre",
    lblMsg: "Message *",
    phMsg: "Décrivez votre projet ou votre demande…",
    send: "Envoyer ma demande",
    sending: "Envoi en cours…",
    successFormspree:
      "Votre message a bien été envoyé. Nous vous répondrons rapidement.",
    successMailto:
      "Votre messagerie va s'ouvrir. Merci d'envoyer l'email pour finaliser.",
    errorSubmit:
      "Une erreur s'est produite. Veuillez réessayer ou nous contacter par téléphone.",
    errName: "Le nom est requis",
    errEmail: "L'email est requis",
    errEmailFmt: "Email invalide",
    errPhone: "Le téléphone est requis",
    errMsg: "Le message est requis",
    faqHowTitle: "Comment fonctionne la conciergerie Zenvilla ?",
    faqHowBody:
      "Zenvilla prend en charge l'intégralité de la gestion de votre villa : accueil des voyageurs, ménage, maintenance, et éventuellement la mise en ligne sur les plateformes de location. Vous suivez tout en temps réel via notre application digitale.",
    faqZonesTitle:
      "Quelles zones couvre Zenvilla en Corse Sud ?",
    faqZonesBody:
      "Nous intervenons sur l'ensemble du Corse Sud : Bonifacio, Porto-Vecchio, Propriano, Sartène, et les alentours. N'hésitez pas à nous contacter pour vérifier la couverture de votre zone.",
    faqPriceTitle:
      "Comment fonctionne la tarification ?",
    faqPriceBody:
      "Nous proposons Zen Tranquillité (essentiel), Zen Intégral (gestion complète) et une formule à la carte pour ne choisir que les prestations utiles. Demandez un devis personnalisé pour votre villa.",
    faqHeading: "FAQ",
    mailtoSubject: "Demande de contact — {name}",
  },
} as const;

export default site;
