const site = {
  Titles: {
    contactPage: "Contacto – Presupuesto e información",
    contactDesc:
      "Contacte con ZenVilla para un presupuesto personalizado. Respuesta rápida todos los días. Propietarios y viajeros en Córcega del Sur.",
    contactOg: "Pida presupuesto o información: respondemos rápido.",
    demanderPage: "Solicitar un servicio – transporte, desayuno, experiencias",
    demanderDesc:
      "Reserve servicios durante su estancia en villa en Córcega del Sur. ZenVilla.",
    demanderOg:
      "Transporte, desayuno, actividades náuticas, equitación… Cuéntenos qué necesita.",
    packsOg:
      "Fórmulas de conserjería para propietarios: esencial, gestión completa u opción a la carte.",
    ownersOg:
      "Conserjería para villas destacadas: acogida, limpieza, anuncios y soporte para huéspedes 7d/7.",
    voyageursOg:
      "Servicios para viajeros: transportes, desayunos, deportes náuticos y alquileres.",
    pvMetaTitle:
      "Conserjería villa Porto-Vecchio – gestión Airbnb y estancias",
    pvMetaDesc:
      "Acompañamos propietarios de villas en Porto-Vecchio, Santa Giulia y Palombaggia.",
    pvOgTitle: "Conserjería villa Porto-Vecchio | ZenVilla",
    pvOgDesc:
      "Confíenos la gestión de su villa con experiencia local e integralidad.",
    sgMetaTitle:
      "Conserjería Santa Giulia – villa y Airbnb Porto-Vecchio",
    sgMetaDesc:
      "Gestión de villas en Santa Giulia: anuncios, huéspedes y limpieza.",
    sgOgTitle: "Conserjería Santa Giulia | ZenVilla",
    sgOgDesc: "Santa Giulia y Porto-Vecchio con servicio completo.",
    mentionsMetaTitle: "Aviso legal",
    mentionsMetaDesc:
      "Información legal del sitio ZenVilla – conserjería Córcega del Sur.",
    politiqueMetaTitle: "Política de privacidad",
    politiqueMetaDesc:
      "Protección de datos (RGPD) y cookies del sitio ZenVilla.",
  },
  Home: {
    heroTitle: "Confíenos las llaves: nos encargamos del resto.",
    heroSubtitle:
      "Nuestra conserjería digital y personalizada lo gestiona todo con total transparencia para que usted pueda despejarse.",
    heroPillars: "REACTIVIDAD • FLEXIBILIDAD • PROXIMIDAD",
    heroAlt: "Bahía de Córcega del Sur – mar turquesa",
    whyTitle: "¿Por qué confiar en ZenVilla?",
    whyLead:
      "Porque gestionar un alquiler a distancia debería ser sencillo. Nuestra conserjería reinventa la gestión de sus villas en Santa Giulia y Porto-Vecchio, con experiencia local única y transparencia total. Usted se libera la mente: nos ocupamos de todo.",
    whyPillars: [
      {
        title: "📍 Cercanía & experiencia",
        body: "Un equipo presente cada día entre Santa Giulia y Porto-Vecchio para una rápida reactividad y mantenimiento 7d/7.",
      },
      {
        title: "🔑 Llave en mano (de la A a la Z)",
        body: "De la creación y optimización del anuncio a la acogida de huéspedes, con limpieza profesional en el medio.",
      },
      {
        title: "🌐 Transparencia",
        body: "Seguimiento riguroso de sus agendas, reservas y del estado de su villa en tiempo real, esté donde esté.",
      },
    ],
    servicesTitle: "Nuestros servicios",
    servicesHint: "Más información →",
    ctaTrustVilla: "Confiar mi villa",
    ownersCtaTitle: "¿Confía su villa?",
    ownersCtaBody:
      "Pida presupuesto: Zen Tranquillité, Zen Integral o fórmula a la carte.",
    reviewsTitle: "Opiniones de huéspedes",
    reviewsLead: "Algunas notas sobre Mini Villa Pinson y la acogida Zenvilla en Santa Giulia.",
    reviewsSource: "Estilo Google · citas ficticias para la maquetación",
    reviews: [
      {
        name: "Claire M.",
        place: "Lyon",
        text: "Villa impecable, a 5 minutos de la playa. Check-in tranquilo y claro.",
      },
      {
        name: "Marco R.",
        place: "Milán",
        text: "Tranquila, limpia, terraza genial. El equipo reservó el kayak al momento.",
      },
      {
        name: "Sophie y Julien",
        place: "París",
        text: "Perfecta en pareja. Llegada fluida, el librito ayuda de verdad.",
      },
      {
        name: "Elena G.",
        place: "Barcelona",
        text: "Santa Giulia como en las fotos. Equipo local, nos sentimos acompañados.",
      },
      {
        name: "Thomas B.",
        place: "Bruselas",
        text: "Limpia a la llegada y a la salida. WhatsApp de principio a fin.",
      },
    ],
  },
  Packs: {
    metaTitlePacks: "Fórmulas – Zen Tranquillité, Zen Integral y a la carte",
    metaDescPacks:
      "Descubra nuestras fórmulas de conserjería: Zen Tranquillité, Zen Integral y a la carte para propietarios en Santa Giulia, Porto-Vecchio y Córcega del Sur.",
    ogTitle: "Nuestras fórmulas | ZenVilla",
    heroTitle: "Nuestras fórmulas",
    heroSubtitle:
      "Elija Zen Tranquillité (esencial), Zen Integral (gestión completa) o a la carte.",
    heroAlt: "Costa corsa – aguas turquesas",
    badgePopular: "Popular",
    zenTTitle: "Zen Tranquillité",
    zenTSubtitle: "Fórmula esencial",
    zenTEssential: [
      "Check-in / check-out de huéspedes",
      "Limpieza integral y preparación de la villa",
      "Gestión del lavado del hogar",
      "Seguimiento con nuestra aplicación digital",
      "Ayuda local si hace falta",
      "Regalo de bienvenida para huéspedes",
    ],
    zenPTitle: "Zen Integral",
    zenPSubtitle: "Fórmula integral",
    zenPEssential: [
      "Toda la fórmula esencial +",
      "Creación y publicación de anuncios Airbnb, Booking, Abritel…",
      "Reportaje fotográfico pro y puesta en valor",
      "Gestión de calendario y reservas",
      "Respuestas a huéspedes 7d/7",
    ],
    carteTitle: "A la carte",
    carteSubtitle: "Las que necesite",
    carteEssential: [
      "Prestaciones modulares: limpieza, ropa blanca, check-in / check-out, reparaciones, mantenimiento…",
    ],
  },
  Owners: {
    metaTitleOwners:
      "Propietarios – conserjería villa Santa Giulia & Porto-Vecchio",
    metaDescOwners:
      "Confíe su villa a ZenVilla: gestión completa y experiencia local.",
    flyerKickerTop: "Conserjería",
    flyerKickerBot: "Córcega del Sur",
    heroTitle: "Confíenos las llaves:\nnosotros hacemos el resto.",
    heroSubtitle:
      "Una conserjería digital y personalizada que lo gestiona todo con total transparencia para que usted pueda tranquilizarse.",
    flyerPillars: "Rapidez • Flexibilidad • Cercanía",
    heroAlt: "Villa Córcega del Sur – Santa Giulia",
    whySectionTitle: "¿Por qué ZenVilla?",
    whyBullets: [
      "📍 Experiencia local única • Santa Giulia & Porto-Vecchio",
      "🔑 Gestión integral de punta a punta • Confíenos las llaves, nos encargamos de todo",
      "🌐 Creación & optimización • Valorización de su anuncio en las plataformas",
      "🗓️ Gestión de reservas • Seguimiento riguroso de agendas y huéspedes",
      "🛎️ Acogida & inventarios • Check-in/out cuidadoso en cada estancia",
      "✨ Limpieza & lavandería • Mantenimiento profesional de alta gama",
      "🛠️ Mantenimiento & reparaciones • Asistencia urgente reactiva 7d/7",
      "👑 Servicios viajero a medida • Prestaciones y actividades personalizadas",
      "🛡️ Conserjería digital • Total transparencia para su tranquilidad",
    ],
    processTitle: "Nuestro alcance para su villa",
    flyerPhases: [
      {
        line1: "Creación & optimización",
        line2: "de su anuncio",
      },
      {
        line1: "Gestión de reservas",
        line2: "& planificación",
      },
      {
        line1: "Acogida, check-in / out",
        line2: "& inventarios",
      },
      {
        line1: "Limpieza profesional",
        line2: "& lavandería",
      },
      {
        line1: "Mantenimiento & reparaciones",
        line2: "urgentes (7d/7)",
      },
      {
        line1: "Servicios & experiencias",
        line2: "personalizadas para huéspedes",
      },
    ],
    advantagesSectionTitle: "Por qué confiar su villa a ZenVilla",
    ctaTitle: "¿Listo para confiar su villa?",
    ctaSubtitle:
      "Pida presupuesto: Zen Tranquillité, Zen Integral o fórmula a la carte.",
  },
  GuestsIndex: {
    metaTitleGuests:
      "Viajeros – servicios y ventajas villa Córcega del Sur",
    metaDescGuests:
      "Prestaciones cuidadas para su estancia en villa: transporte, desayuno, deportes náuticos, alquileres.",
    heroTitle: "Servicios para viajeros",
    heroSubtitle:
      "Máximo confort en su villa en Córcega del Sur.",
    heroAlt: "Costa mediterránea – mar turquesa",
    servicesHeading: "Servicios para su estancia",
    hint: "Más información →",
    ctaTitle: "¿Una estancia a medida?",
    ctaBody:
      "Salidas en barco, reparto de compras, paseos a caballo… Cuéntenos qué busca y le ayudamos.",
  },
  DemanderPrestation: {
    heroTitle: "Solicitar un servicio",
    heroSubtitle:
      "Transporte, desayuno, actividades náuticas… Díganos qué necesita.",
    formTitle: "Formulario de solicitud",
    fieldService: "Servicio deseado *",
    optionPlaceholder: "Seleccione un servicio",
    fieldName: "Nombre *",
    fieldEmail: "Email *",
    fieldPhone: "Teléfono *",
    nomPlaceholder: "Su nombre",
    emailPlaceholder: "su@email.es",
    phonePlaceholder: "6 12 34 56 78",
    fieldPlace: "Villa o lugar de estancia",
    fieldDates: "Fechas deseadas",
    fieldMsg: "Mensaje / detalles",
    submit: "Enviar mi solicitud",
    sending: "Enviando…",
    successFormspree:
      "Mensaje enviado: le contactaremos pronto.",
    successMailto:
      "Se abrirá su correo: envíe el mensaje para validar.",
    error:
      "Error al enviar. Inténtelo de nuevo o llámenos.",
    sideTitle: "¿Una necesidad concreta?",
    sideParagraph:
      "Escríbanos o llamen: presupuesto en 24–48 h.",
    guestLabel: "Viajero — servicio",
    bannerAlt: "Servicios para viajeros – Córcega del Sur",
    asideTitleDirect: "O contáctenos directamente",
    asideLead:
      "¿Prefiere llamada o correo? Estamos para responder sus preguntas.",
    callCaption: "Llámenos",
    emailCaption: "Envíenos un correo",
    whatsappCaption: "WhatsApp",
    whatsAction: "Contactarnos",
    placePlaceholder: "Santa Giulia, Porto-Vecchio…",
    datesPlaceholder: "Ej.: 15–22 de julio",
    msgPlaceholder: "Describa su necesidad…",
    errSend:
      "Se produjo un error. Use email o teléfono en el panel derecho.",
  },
  Legal: {
    mentionsLead:
      "Información facilitada conforme a los artículos 6-III y 19 de la ley francesa núm. 2004-575 del 21 de junio de 2004 para la economía digital (LEN).",
    lastUpdatedPrefix: "Última actualización:",
  },
  ConciergePV: {
    heroTitle: "Conserjería villa Porto-Vecchio",
    heroSubtitle:
      "Gestión integral en Santa Giulia, Palombaggia y Porto-Vecchio.",
    heroImgAlt: "Conserjería villa Porto-Vecchio – Santa Giulia",
    bodyTitle: "Conserjería villa Porto-Vecchio – ZenVilla",
    bodyLead:
      "ZenVilla es su conserjería de referencia en Porto-Vecchio y Córcega del Sur. Gestionamos su villa en Santa Giulia, Palombaggia y alrededores: anuncios Airbnb y Booking, check-in/out, limpieza, mantenimiento y asistencia 7d/7.",
    bodyNext:
      "Confíenos su propiedad para alquileres serenos durante todo el año.",
    ctaTitle: "¿Villa en Porto-Vecchio? Hablemos",
  },
  ConciergeSG: {
    heroTitle: "Conserjería villa Santa Giulia",
    heroSubtitle:
      "Experiencia local para su villa en Santa Giulia y playas emblemáticas.",
    heroImgAlt: "Conserjería villa Santa Giulia",
    bodyTitle: "Santa Giulia – gestión de villas en Córcega",
    bodyLead:
      "Santa Giulia es una de las playas más bellas de Córcega. ZenVilla ofrece conserjería completa: Airbnb y Booking, acogida, limpieza y disponibilidad 7d/7.",
    bodyNext:
      "Nuestro equipo conoce Santa Giulia y Porto-Vecchio: confíe en nosotros.",
    ctaTitle: "¿Villa en Santa Giulia? Confíela a ZenVilla",
  },
  ContactPage: {
    heroTitle: "Contacto",
    heroSubtitle:
      "¿Una pregunta o un proyecto? Respondemos rápido.",
    heroAlt:
      "Contactar ZenVilla – aguas turquesas Córcega del Sur, conserjería villas",
    fastReply: "Respuesta rápida – 7d/7",
    formTitle: "Pedir presupuesto",
    lblName: "Nombre *",
    phName: "Su nombre",
    lblEmail: "Email *",
    phEmail: "su@email.es",
    lblPhone: "Teléfono *",
    phPhone: "6 12 34 56 78",
    lblCity: "Ciudad / zona en Córcega",
    phCity: "Bonifacio, Porto-Vecchio…",
    lblType: "Tipo de propiedad",
    typeOpt: "Seleccione",
    typeVilla: "Villa",
    typeApt: "Apartamento",
    typeMaison: "Casa",
    typeAutre: "Otro",
    lblMsg: "Mensaje *",
    phMsg: "Describa su proyecto…",
    send: "Enviar mi solicitud",
    sending: "Enviando…",
    successFormspree:
      "Mensaje enviado: le responderemos pronto.",
    successMailto:
      "Se abrirá su cliente de correo: envíe el email para finalizar.",
    errorSubmit:
      "Se produjo un error. Inténtelo de nuevo o llámenos.",
    errName: "El nombre es obligatorio",
    errEmail: "El email es obligatorio",
    errEmailFmt: "Email no válido",
    errPhone: "El teléfono es obligatorio",
    errMsg: "El mensaje es obligatorio",
    faqHowTitle: "¿Cómo funciona la conserjería ZenVilla?",
    faqHowBody:
      "ZenVilla gestiona toda la villa: acogida, limpieza, mantenimiento y, si es preciso, publicación en plataformas. Siga todo en tiempo real con nuestra app.",
    faqZonesTitle: "¿Qué zonas cubre ZenVilla?",
    faqZonesBody:
      "Intervenimos en Córcega del Sur: Bonifacio, Porto-Vecchio, Propriano, Sartène y alrededores. Pregunte por su zona.",
    faqPriceTitle: "¿Cómo es la tarificación?",
    faqPriceBody:
      "Ofrecemos Zen Tranquillité (esencial), Zen Integral (gestión completa) y fórmula a la carte. Pida un presupuesto personalizado.",
    faqHeading: "FAQ",
    mailtoSubject: "Solicitud de contacto — {name}",
  },
} as const;

export default site;
